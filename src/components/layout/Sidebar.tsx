...
// ── Nav items defined OUTSIDE the component so React never recreates the type ──
const NAV_CORE = [
...
// ── Extracted NavItem — NOT inside Sidebar so React keeps a stable reference ──
function NavItem({
      ...
      <aside className={cn(
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 scrollbar-hide space-y-6">
          <div className="space-y-0.5">
            {!sidebarCollapsed && (
              <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50 px-3 pb-2">Menu</p>
            )}
            {NAV_CORE.map((item) => (
              <NavItem
                key={item.href}
          ...
          <div className="space-y-0.5">
            {!sidebarCollapsed && (
              <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50 px-3 pb-2">Account</p>
            )}
          ...
          {!sidebarCollapsed && (
            <div className="flex items-center gap-2.5 px-2 py-2 rounded-xl mb-1">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold truncate leading-none">{profile.name}</p>
